...
ganr_0 <- mutate(ganr_0, run = gsub("out", "", run)) %>%
  mutate(run = as.integer(run))
ganr_1 <- mutate(ganr_1, run = gsub("out", "", run)) %>%
  mutate(run = as.integer(run) + max(ganr_0$run))
ganr_2 <- mutate(ganr_2, run = gsub("asex", "", run)) %>%
  mutate(run = as.integer(run) + max(ganr_1$run))
ganr_3 <- mutate(ganr_3, run = 1, rr = "1E-6", mu = "1E-3", perc = "0.35") %>%
  mutate(run = as.integer(run) + max(ganr_2$run))
ganr_4 <- mutate(ganr_4, run = 1, rr = "1E-6", mu = "1E-4", perc = "0.35") %>%
  mutate(run = as.integer(run) + max(ganr_3$run))
ganr_5 <- mutate(ganr_5, run = 1, rr = "1E-6", mu = "1E-2", perc = "0.35") %>%
  mutate(run = as.integer(run) + max(ganr_4$run))
...
hz_0 <- mutate(hz_0, run = gsub("out", "", run)) %>%
  mutate(run = as.integer(run))
hz_1 <- mutate(hz_1, run = gsub("out", "", run)) %>%
  mutate(run = as.integer(run) + max(ganr_0$run))
hz_2 <- mutate(hz_2, run = gsub("asex", "", run)) %>%
  mutate(run = as.integer(run) + max(ganr_1$run))
hz_3 <- mutate(hz_3, run = 1, rr = "1E-6", mu = "1E-3", perc = "0.35") %>%
  mutate(run = as.integer(run) + max(ganr_2$run))
hz_4 <- mutate(hz_4, run = 1, rr = "1E-6", mu = "1E-4", perc = "0.35") %>%
  mutate(run = as.integer(run) + max(ganr_3$run))
hz_5 <- mutate(hz_5, run = 1, rr = "1E-6", mu = "1E-2", perc = "0.35") %>%
  mutate(run = as.integer(run) + max(ganr_4$run))
...
all_ <- bind_rows(hz_0, hz_1) %>%
  ...
  mutate(just = ifelse((Moment == "nonb" & lead(Moment) != "nonb"),
                       "pre", "uninf")) %>%
  mutate(just = ifelse((Moment == "nonb" & lag(Moment) != "nonb"),
                       "post", just)) %>%
  ungroup() %>%
  rename(H = value) %>%
